...
3. `Text-Guided Region Cross Encoder`：完整 `text_tokens` 为 Q，P3 feature 为 K/V；四层 8-head Cross-Attention 更新文本 token，再逐格输出 region logits，每个样本取 top-8 P3 cell（约 128 cm²）。
4. Anchor：top-8 cell 展开到 P1 active voxel，经 FPS 得到最多 48 个 Query；不足部分由 `query_valid_mask` padding。Query 初始化为 `LayerNorm(P1 feature + world-coordinate PE)`。
...
10. 集合输出：48 个 raw boxes 经 score 排序与 pose NMS，输出最多 16 个 Box；所有 Box Size 严格复制 Source Size。
...
- 粗区域使用完整 `text_tokens` 与 P3 feature。
