#!/usr/bin/env python
"""
tools/convert_housecat_to_canonical.py
---------------------------------------
将 HouseCat6D 数据离线转换为 canonical placement scene 数据集。

使用示例:
    conda run -n spatial python tools/convert_housecat_to_canonical.py \
        --root-dir /data/wenhao.hai/housecat6D/housecat6D_trainset \
        --output-dir data/housecat \
        --depth-window-cm 50 \  
        --frame-step 10 --point-cloud-stride 2
...
from src.datasets.housecat_to_canonical import (
    ...
    HouseCatCanonicalConverter,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert HouseCat6D frames to canonical placement scene dataset.",
    )
    parser.add_argument(
        "--root-dir",
        default="/data/wenhao.hai/housecat6D/housecat6D_trainset",
        help="HouseCat6D 训练集根目录（包含 scene01/ scene02/ … 子目录）。",
    )
    parser.add_argument(
        "--output-dir",
        default="/data/jiajun.xie/3D_Box/data/housecat",
...
def main() -> None:
    args = parse_args()
    converter = HouseCatCanonicalConverter(
    ...
    if manifest["preprocess"].get("skip_count", 0) > 0:
        print(f"Skipped {manifest['preprocess']['skip_count']} frames (support plane failed)")
