...
严格筛选要求 Ours 的 benchmark `task_success=true`，且 RoboBrain2.5 的候选框与源物体之外的场景物体发生碰撞。RoboBrain2.5 只输出二维点；图中的红色三维候选框使用该点及源物体的观测尺寸、朝向构造，仅用于可视化。碰撞结论复用 benchmark 的 OBB 碰撞检测，蓝色框为 LC-BGPlaceNet 的预测放置框。
